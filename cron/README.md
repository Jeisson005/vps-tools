...
- `nightly_refresh.sh`: Nightly restart of `opencode-web`, `open-webui`, `rustdesk-web`, `wa-jeisson` (MCP WhatsApp, con `GET /status` health-check) + `hermes-gateway.service` (puente del agente, con `GET /health` health-check) + light cleanup (docker builder cache, /tmp leftovers, journal vacuum). Never touches `steel-*`, `sentinel`, `nginx`, `headscale`. Opt-out por puente vía `REFRESH_WHATSAPP_MCP=false` / `REFRESH_HERMES_GATEWAY=false` en `cron/.env`.
