...
class AiClient:
    def __init__(self, base_url: str = "", api_key: str = "", model: str = "", session_id: str = ""):
        ...
        # OpenCode Go requires a stable session id per conversation to route and
        # cache efficiently. Derive a stable one from the account so it survives
        # gateway restarts; callers may override it via config.
        self.session_id = session_id or uuid.uuid5(
            uuid.NAMESPACE_URL, f"mcp-gateway:{self.base_url}:{self.model}:{self.api_key}"
        ).hex
    ...
    def _headers(self) -> dict:
        headers = {
            ...
            "User-Agent": "vps-mcp-gateway/1.0",
        }
        # Only OpenCode Go/Zen needs (and understands) the session header.
