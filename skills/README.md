...
| **`user-task-management`** | OpenCode & Hermes | Personal task management in ClickUp: smart list routing, mandatory time estimate + due date + priority (asks when missing), descriptions, subtasks and checklists. Reads autonomous, writes with confirmation. | `clickup` via MCP Gateway (`:8005/clickup`) |
